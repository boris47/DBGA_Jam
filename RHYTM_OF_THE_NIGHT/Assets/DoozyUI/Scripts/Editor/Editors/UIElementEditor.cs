...
#pragma warning restore 0414
        ...
        void ValidateElementCategoryAndElementName()
        {
            ...
            {
                elementCategoryIndex.index = DatabaseUIElements.CategoryNameIndex(uiElement.elementCategory);
                elementNameIndex.index = DatabaseUIElements.ItemNameIndex(uiElement.elementCategory, uiElement.elementName);
            }
        }
        ...
        void DrawSettings(float width)
        {
            ...
            QUI.BeginHorizontal(width);
            {
                ...
                QLabel.style = Style.Text.Normal;
                tempFloat = width - QLabel.x - 16; //property field width

                QUI.Box(QStyles.GetBackgroundStyle(Style.BackgroundType.Low, selectedButton.objectReferenceValue != null ? QColors.Color.Blue : QColors.Color.Gray), width, 20);
                QUI.Space(-width + SPACE_4);

                QUI.Label(QLabel);

                QUI.PropertyField(selectedButton, tempFloat);
                        ...
                        float width)
        {
            ...
            DUIUtils.DrawSound(target, "sound @START", customSoundAtStart, soundAtStart, soundAtStartIndex, width);
            DUIUtils.DrawSound(target, "sound @FINISH", customSoundAtFinish, soundAtFinish, soundAtFinishIndex, width);
        }
        ...
        void LoadPreset(AnimType animType)
        {
            if(serializedObject.isEditingMultipleObjects)
            {
                ...
                switch(animType)
                {
                    ...
                    case AnimType.Loop:
                        ...
                        for(int i = 0; i < targets.Length; i++) { UIElement iTarget = (UIElement)targets[i]; iTarget.loopAnimations = loopAnim.Copy(); }
                        break;
                }
            }
            else
            {
                ...
                switch(animType)
                {
                    ...
                    case AnimType.Loop: uiElement.loopAnimations = UIAnimatorUtil.GetLoop(loopAnimationsPresetCategoryName.stringValue, loopAnimationsPresetName.stringValue).Copy(); break;
                }
            }
            QUI.ExitGUI();
        }

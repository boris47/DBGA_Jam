...
public class DragAndDropItem : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public void OnDrag(PointerEventData data)
    {
		if (icon != null)
        {
           // icon.transform.position = Input.mousePosition.x;   // Item's icon follows to cursor
			icon.transform.position = new Vector2(Input.mousePosition.x, sourceCell.transform.position.y);
        }
    }
